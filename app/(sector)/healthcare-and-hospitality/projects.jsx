...
export default function Blogs() {
  ...
  React.useEffect(() => {
    const fetchBlogs = async () => {
      const query = `*[_type == "post" && (categories[]->title match "Healthcare & Pharmaceuticals" || categories[]->title match "Hospitality")] | order(publishedAt desc) {
        _id,
        title,
        publishedAt,
        mainImage{
      ...
      <div className="container pb-50">
        <div className="row align-items-center">
          <div className="col-md-6 text-start">
            <div className="title-area">
              <h2 className="sec-title" style={{ color: "#081761", textDecoration: "underline" }}>
                Explore Our Healthcare & Hospitality Projects
              </h2>
            </div>
          </div>
          <div className="col-md-6 text-end pb-5">
            <Link
              scroll={false}
              href="/portfolio?category=Healthcare+%26+Hospitality"
              className="btn"
              style={{
