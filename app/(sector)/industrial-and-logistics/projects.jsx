...
export default function Blogs() {
  ...
  React.useEffect(() => {
    const fetchBlogs = async () => {
      const query = `*[_type == "post" && (categories[]->title match "Industrial" || categories[]->title match "Logistics & Warehouses")] | order(publishedAt desc) {
        _id,
        title,
        publishedAt,
        mainImage{
  ...
  const industrialProjects = blogs.filter(post => 
    post.categories.some(cat => cat.title === "Industrial")
  );
  const logisticsProjects = blogs.filter(post => 
    post.categories.some(cat => cat.title === "Logistics & Warehouses")
  );

  // Take 3 from each category (or less if not enough)
  const displayProjects = [
    ...industrialProjects.slice(0, 3),
    ...logisticsProjects.slice(0, 3)
  ].slice(0, 6); // Ensure we don't exceed 6 total
      ...
      <div className="container pb-50">
        <div className="row align-items-center">
          ...
          <div className="col-md-6 text-end pb-5">
            <Link
              scroll={false}
              href="/portfolio?category=Industrial+%26+Logistics"
              className="btn"
              style={{
